...
var ATT = {
	/*
	 * SEND SMS @param body- Will contain two values Message and Address @param
	 * contentType- format in which data is to be sent Example: contentType @
	 * application/json 'body':{ "Message":"Test AT&T
	 * Sms","Address":"tel:+XXXXXXXXXX,tel:XXXXXXXXXX"} contentType @
	 * application/xml 'body':"<SmsRequest><Address>tel:XXXXXXXXXX,tel:XXXXXXXXXX</Address><Message>Test
	 * XML</Message></SmsRequest>" contentType @
	 * application/x-www-form-urlencoded SAMPLE : test = 'Address=' + number +
	 * '&' + 'Message=' +message; urlbody = encodeURI(test);
	 * 'body':"Address=tel%3AXXXXXXXXXX&Message=URL%20ENCODED" for accept =
	 * application/xml var smsId = $(data).attr('id'); for accept
	 * =application/json var smsId = JSON.parse(data).Id;
	 */
	...
	sendSMS : function(number, message, cbData) {
		...
		phonegap.ATT.SMS.sendSMS({
			...
			var smsId = JSON.parse(data).outboundSMSResponse.messageId;
			cbData(data, smsId);
		}, function(error) {
	...
	/*
	 * GET SMS DELIVERY STATUS @param smsId -SmsId received in sendSMS response -
	 * Mandatory
	 */
	...
	/*
	 * GET SMS @param registrationID -RegistrationID received when application
