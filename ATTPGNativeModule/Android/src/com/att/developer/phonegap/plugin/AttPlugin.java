...
import com.att.developer.phonegap.plugin.services.ServiceWap;
...
public class AttPlugin extends CordovaPlugin {
	...
	private Service getService(String action) {
		...
		} else if (action.equalsIgnoreCase(AttPluginConstants.ACTION_WAP)) {
			return new ServiceWap();
	...
	private WeakHashMap<String, String> prepareArguments(String action, JSONArray args) throws JSONException, Exception {
		...
		try {
			if(AttPluginConstants.ACTION_WAP.equals(action))
			{
				...
				argValues.put(AttPluginConstants.ARG_DATA,data);
			...
			else if(AttPluginConstants.ACTION_SPEECH.equals(action))
			{
				JSONObject json=args.getJSONObject(0);
				argValues.put(AttPluginConstants.ARG_URL, json.getString("url"));
