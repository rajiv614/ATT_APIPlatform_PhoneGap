		...
		var getMessageHeaders = function() {
			phonegap.ATT.MOBO
					.getMessageHeaders(
							...
							function(data) {
								messageId = JSON.parse(data).MessageHeadersList.Headers[0].MessageId;
								contentType = JSON.parse(data).MessageHeadersList.Headers[0].MmsContent
										&& JSON.parse(data).MessageHeadersList.Headers[0].MmsContent[1].ContentType;
								console
										.log('MOBO GetMessageHeaders Successfully Called : : '
												+ data);
								cbData(data);

							}, function(error) {
								console.log('MOBO GetMessageHeaders Error : '
										+ error);
								cbData(JSON.stringify(error));
							});
		};
	...
	MOBOGetMessageContent : function(cbData) {
		var getMessageContent = function() {
			phonegap.ATT.MOBO.getMessageContent({
				...
				console.log('MOBO GetMessageContent Successfully Called : '
						+ data);
				var response = 'RESPONSE Is: ' + 'Length:' + data.length
