		...
		function(require, exports, module) {
			...
			var httpRequest = {
				createHttpObject : function(object, cbOnData, cbOnError) {
					console.log('jQuery Request Object :'
							+ JSON.stringify(object));
					object["success"] = function(response) {
						if (object.headers.Accept === 'application/xml') {
							var xmlResponse = new XMLSerializer()
					...
					object["error"] = function(xhr, status, errThrown) {
						...
						out += "<br>Error request status text: "
								+ xhr.statusText;
						out += "<br>Error request status: " + xhr.status;
						out += "<br>Error request response text: "
								+ xhr.responseText;
						out += "<br>Error response header: "
			...
			bedFrame.build = function bedFrameTransformObject(target, api) {
				...
				for (c in children) {
					...
					var propertyTypes = child.propertyTypes
							|| api.propertyTypes || {};
					...
					for ( var o in api) {
						...
						case BedFrame.PROPERTY_TYPE_ONLY_LATEST:
							...
							child[o] = child[o] === undefined ? api[o]
									: child[o];
							break;
					...
					// recurse upon the sub-segment of the API.
						...
						bedFrameTransformObject(target[child.property] = {},
								child);
					}
				}
			};
			...
			ATT.Constants = {
				...
				ENDPOINT_ACCESS_TOKEN : 'https://api.att.com/oauth/access_token',
				ENDPOINT_SMS : 'https://api.att.com/rest/sms/2/messaging',
				ENDPOINT_STT : 'https://api.att.com/rest/1/SpeechToText',
				ENDPOINT_NOTARY : 'https://api.att.com/Security/Notary/Rest/1/SignedPayload',
				ENDPOINT_MMS : 'https://api.att.com/rest/mms/2/messaging',
				ENDPOINT_OAUTH : 'https://api.att.com/oauth',
				ENDPOINT_WAPPUSH : 'https://api.att.com/1/messages/outbox/wapPush',
				ENDPOINT_LOCATION : 'https://api.att.com/2/devices/location',
				ENDPOINT_DEVICECAPABILITIES : 'https://api.att.com/1/devices/',
				ENDPOINT_MOBO : 'https://api.att.com/rest/1/MyMessages',
				ENDPOINT_ROOT_PAYMENT : 'https://api.att.com',
			...
			var generateAccessToken = function(callBackToken) {
				...
				if ((sessionOBJ.isOAuth_Code)
						&& (sessionOBJ.grantType === 'authorization_code')
						&& (sessionOBJ.oauthCode !== null)) {
					body = 'client_id=' + sessionOBJ.accessKeyId
							+ '&client_secret=' + sessionOBJ.secretKey
							+ '&grant_type=' + sessionOBJ.grantType + '&code='
							+ sessionOBJ.oauthCode;
				} else {
					if (sessionOBJ.grantType === 'client_credentials') {
						body = 'client_id=' + sessionOBJ.accessKeyId
								+ '&client_secret=' + sessionOBJ.secretKey
								+ '&grant_type=' + sessionOBJ.grantType
				...
				var jquery_object = {
					...
					beforeSend : function(xhr) {
						xhr.setRequestHeader('Content-Type',
								ATT.Constants.CONTENT_TYPE_URL_ENCODED);

					}
				};
				httpRequest
						.createHttpObject(jquery_object, cbOnData, cbOnError);
			...
			var SMSExecutor = function(params, cbOnData, cbOnError) {
				...
				var smsRequest = function(accessToken) {
					...
					if (params.smsId !== undefined) {
						...
						var getSMSEndPoint = (self.endPoint + self.appendUrl)
			...
			var NOTARYExecutor = function(params, cbOnData, cbOnError) {
				...
				notaryRequest = function(accessToken) {
					if (params.data.MerchantPaymentRedirectUrl !== undefined) {
						params.data.MerchantPaymentRedirectUrl = params.data.MerchantPaymentRedirectUrl
			...
			var STTExecutor = function(params, cbOnData, cbOnError) {
				...
				sttRequest = function(accessToken) {
					...
					var args = [ {
						...
						contentLength : ((params.contentLength === undefined) ? ""
								: params.contentLength),
						XSpeechContext : ((params.xSpeechContext === undefined) ? "Generic"
								: params.xSpeechContext),
						transferEncoding : ((params.transferEncoding === undefined) ? ""
								: params.transferEncoding),
						url : self.endPoint
					} ];
			...
			var SendWAPPushExecutor = function(params, cbOnData, cbOnError) {
				sessionOBJ.attHelper.prepareExecutor(this);
				var self, wapRequest, args;
				self = this;
			...
			var LOCATIONExecutor = function(params, cbOnData, cbOnError) {
				...
				locationRequest = function(accessToken) {
					if ((params.requestedAccuracy !== undefined)
							|| (params.tolerance !== undefined)
							|| (params.acceptableAccuracy !== undefined)) {
				 ...
				 * oauth token. CALLING function generate AccessToken
				 */
			...
			var MMSExecutor = function(params, cbOnData, cbOnError) {
				...
				mmsRequest = function(accessToken) {
					if (self.method.toLowerCase() === 'getmmsdeliverystatus') {
						...
						var token = 'Bearer' + ' ' + accessToken;
						var types = [ {
							body : (params.body !== undefined) ? params.body
									: '',
							accessToken : token,
							url : self.endPoint + self.appendUrl,
			...
			var PaymentExecutor = function(params, cbOnData, cbOnError) {
				...
				sessionOBJ.attHelper.prepareExecutor(this);
				if (self.method.toLowerCase() === 'newtransaction'
						|| self.method.toLowerCase() === 'newsubscription') {
					cbOnData(this.endPoint + this.appendUrl + '?Signature='
							+ params.signature + "&SignedPaymentDetail="
							+ params.signedDocument + "&clientid="
							+ params.clientId);
					...
					getStatus = function(accessToken) {
						...
						} else if (self.method.toLowerCase() === 'getnotification'
								|| self.method.toLowerCase() === 'acknowledgenotification') {
							jquery_object.url = self.endPoint + self.appendUrl
									+ params.notificationId;
						} else if (self.method.toLowerCase() === 'getsubscriptiondetails') {
							jquery_object.url = self.endPoint + self.appendUrl
									+ params.merchantSubscriptionId
									+ '/Detail/' + params.consumerId;
						}
						httpRequest.createHttpObject(jquery_object, cbOnData,
								cbOnError);
			 ...
			 */
			var MOBOExecutor = function(params, cbOnData, cbOnError) {
				sessionOBJ.attHelper.prepareExecutor(this);
				var self = this, moboRequest;
				...
				moboRequest = function(accessToken) {
						...
						if (self.method.toLowerCase() === 'getmessageheaders') {
							if (params.indexCursor !== undefined) {
								jquery_object.url = self.endPoint
										...
										+ '&'
										+ 'IndexCursor='
										+ encodeURIComponent(params.indexCursor);
							} else {
								jquery_object.url = self.endPoint + '?'
					...
					/*
					 * set sessionOBJ.isOAuth_Code = true to indicate generation
					 * of oauth token. CALLING function generate AccessToken
					 */
				};
				sessionOBJ.isOAuth_Code = true;
			...
			ATT.authorize = function(accessKeyId, secretKey, scope, grantType,
					oauthCode) {
			...
			sessionOBJ.bedFrame
					.build(
							ATT,
							{
								...
								preparer : function() {
									if (!this.action) {
										var initCap = this.method.substr(0, 1)
												.toUpperCase();
										this.action = initCap
												+ this.method.substr(1);
								...
								children : [
										...
										{
											property : 'WAPPush',
											endPoint : ATT.Constants.ENDPOINT_WAPPUSH,
										...
										{
											property : 'MOBO',
											endPoint : ATT.Constants.ENDPOINT_MOBO,
