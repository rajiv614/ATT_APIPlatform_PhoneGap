 ...
 * 
 * @license RequireJS 2.0.4+ Copyright (c) 2010-2012, The Dojo Foundation All
...
(function(global) {
	'use strict';
	var version = '2.0.4+', commentRegExp = /(\/\*([\s\S]*?)\*\/|([^:]|^)\/\/(.*)$)/mg, cjsRequireRegExp = /[^.]\s*require\s*\(\s*["']([^'"\s]+)["']\s*\)/g, jsSuffixRegExp = /\.js$/, currDirRegExp = /^\.\//, ostring = Object.prototype.toString, ap = Array.prototype, aps = ap.slice, apsp = ap.splice, isBrowser = !!(typeof window !== 'undefined'
			&& navigator && document), isWebWorker = !isBrowser
	...
	isOpera = typeof opera !== 'undefined'
			&& opera.toString() === '[object Opera]', contexts = {}, cfg = {}, globalDefQueue = [], useInteractive = false, req, s, head, baseElement, dataMain, src, interactiveScript, currentlyAddingScript, mainScript, subPath;
	...
	function addRequireMethods(req, context, relMap) {
		each([ [ 'toUrl' ], [ 'undef' ], [ 'defined', 'requireDefined' ],
				[ 'specified', 'requireSpecified' ] ], function(item) {
			var prop = item[1] || item[0];
			req[item[0]] = context ? makeContextModuleFunc(context[prop],
					relMap) :
	...
	function newContext(contextName) {
		...
		function normalize(name, baseName, applyMap) {
			var baseParts = baseName && baseName.split('/'), normalizedBaseParts = baseParts, map = config.map, starMap = map
					&& map['*'], pkgName, pkgConfig, mapValue, nameParts, i, j, nameSegment, foundMap;
		...
		function makeModuleMap(name, parentModuleMap, isNormalized, applyMap) {
			var index = name ? name.indexOf('!') : -1, prefix = null, parentName = parentModuleMap ? parentModuleMap.name
					: null, originalName = name, isDefine = true, normalizedName = '', url, pluginModule, suffix;
		...
		function takeGlobalQueue() {
			...
			if (globalDefQueue.length) {
				...
				// on context.
				apsp.apply(defQueue, [ defQueue.length - 1, 0 ]
		...
		function makeRequire(mod, enableBuildCallback, altRequire) {
			var relMap = mod && mod.map, modRequire = makeContextModuleFunc(
					altRequire || context.require, relMap, enableBuildCallback);
		...
		function checkLoaded() {
			...
			expired = waitInterval
					&& (context.startTime + waitInterval) < new Date()
							.getTime(), noLoads = [], stillLoading = false, needCycleCheck = true, map, modId, err, usingPathFallback;
		...
		Module.prototype = {
			...
			fetch : function() {
				...
				if (this.shim) {
					makeRequire(this, true)(
							this.shim.deps || [],
							bind(this, function() {
								return map.prefix ? this.callPlugin() : this
										.load();
							}));
				} else {
					// Regular dependency.
						...
						if (isFunction(factory)) {
							...
							if (this.map.isDefine) {
								...
								if (cjsModule
										...
										// Make sure it is not already the
										// exports value
			...
			callPlugin : function() {
				var map = this.map, id = map.id, pluginMap = makeModuleMap(
						map.prefix, null, false, true);
				...
				on(
						...
						bind(
								this,
								function(plugin) {
									var name = this.map.name, parentName = this.map.parentMap ? this.map.parentMap.name
											: null, load, normalizedMap, normalizedMod;
									...
									load.fromText = function(moduleName, text) {
										...
										// Turn off interactive script matching
										// for IE for any define
										// calls in the text, then turn it back
										// on at the end.
										if (hasInteractive) {
											useInteractive = false;
										}

										// Prime the system by creating a module
										// instance for
										// it.
									...
									plugin.load(map.name, makeRequire(
											map.parentMap, true, function(deps,
													cb) {
		...
		return (context = {
			...
			completeLoad : function(moduleName) {
				var shim = config.shim[moduleName] || {}, shExports = shim.exports
						&& shim.exports.exports, found, args, mod;
			...
			toUrl : function(moduleNamePlusExt, relModuleMap) {
				...
				return context.nameToUrl(normalize(moduleNamePlusExt,
						relModuleMap && relModuleMap.id, true), ext);
			},
			...
			onScriptError : function(evt) {
				...
				if (!hasPathFallback(data.id)) {
					return onError(makeError('scripterror', 'Script error',
							evt, [ data.id ]));
				}
			}
		});
	...
	req.load = function(context, moduleName, url) {
		...
		if (isBrowser) {
			...
			node = config.xhtml ? document.createElementNS(
