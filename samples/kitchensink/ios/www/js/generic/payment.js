...
var MerchantPaymentRedirectUrl = window.localStorage.getItem("MerchantPaymentRedirectUrl");
var Amount = "0.99";
var Category = 1;
...
var ATT = {
	...
	notary : function(notaryData, cbData) {
	...
	transaction : function(type) {
		var date = new Date(), ticks = date.getTime(), min = 1000000000, max = 9999999999, 
		...
		if (type === "transaction") {
			ATT.notary(notaryData, ATT.newTransaction);
		} else if (type === "subscription") {
			//Add additional parameters for subscription
			$.extend(notaryData, {
			...
			ATT.notary(notaryData, ATT.setSubscription);
		}
	},
			...
			$("#transactionFrame").load(function() {
				...
				if (url.indexOf('success') !== -1) {
					...
					if (url.indexOf('TransactionAuthCode') !== -1) {
						...
						window.localStorage.TransactionAuthCode = url.substr(index + 20, url.length + 1);
						$("#btntransactionstatus").attr("disabled", false);
						$("#iframe").hide();
					}
				}
			});
	...
	gettransactionstatus : function(OauthCode) {
		phonegap.ATT.Payment.getTransactionStatus({
			"transactionAuthCode" : OauthCode
		}, function(data) {

			var result = JSON.parse(data);
			if (result.TransactionId.length > 19) {
				transacId = result.TransactionId.substring(0, 18) + '..';
			} else {
				transacId = result.TransactionId;
			}
			var newRow = $("<tr class=\"txrow\" id=\"" + result.TransactionId + "\"><td align=\"left\" class=\"txcolleft\">" + transacId + "</td>" + "<td align=\"right\" class=\"txcolright\"><input type=\"radio\" name=\"rdtransaction\"></td></tr>");
	...
	refundtransaction : function() {
		phonegap.ATT.Payment.refundTransaction({
	...
	setSubscription : function(signedDocument, signature) {
		phonegap.ATT.Payment.newSubscription({
			"signedDocument" : signedDocument,
			"signature" : signature,
			'clientId' : accessKey
		}, function(data) {
			...
			$("#transactionFrame").load(function() {
				...
				if (url.indexOf('success') != -1) {
					...
					if (url.indexOf('SubscriptionAuthCode') !== -1) {
						...
						window.localStorage.subscriptionAuthCode = url.substr(index + 21, url.length + 1);
	...
	getSubscriptionStatus : function() {
		phonegap.ATT.Payment.getSubscriptionStatus({
			...
			var newRow = $("<tr class=\"txrow\" id=\"" + result.SubscriptionId + "\"><td align=\"left\" class=\"txcolleft\">" + subscrptnId + "</td>" + "<td align=\"right\" class=\"txcolright\"><input type=\"radio\" name=\"rdtransaction\"></td></tr>");
			$("#txtable").append(newRow);
			newRow.find('td input:radio').click(function() {
	...
	getSubscriptionDetails : function() {
		phonegap.ATT.Payment.getSubscriptionDetails({
			...
			console.log("GetSubscriptionDetails Success:" + data);
	...
	acknowledgeNotification : function(notificationId) {
		phonegap.ATT.Payment.acknowledgeNotification({
			...
			$("#notificationtable tr").each(function() {
				if (this.id == notificationId) {
					...
					$("#notificationIdToBeRemoved").val(originalList + notificationId);
				}
			});
		}, function(error) {
