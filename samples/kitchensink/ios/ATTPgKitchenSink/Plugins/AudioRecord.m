...
#pragma unused(callbackId)
    ...
    NSMutableDictionary* options = [arguments objectAtIndex:2];
	NSString* mediaId = [arguments objectAtIndex:0];
    
    NSLog(@"File : %@",[arguments objectAtIndex:1]);
    //Use Super's audio file.
	CDVAudioFile* audioFile = [super audioFileForResource:[arguments objectAtIndex:1] withId: mediaId doValidation:YES forRecording:YES ];
	...
	if (audioFile != nil) {
        ...
        NSLog(@"Audio FilePath : %@",audioFile.resourceURL);
...
#pragma unused(callbackId)
	...
	CDVAudioFile* audioFile = [super audioFileForResource:[command.arguments objectAtIndex:1] withId:mediaId doValidation:YES forRecording:NO ];
    NSString* jsString = nil;
